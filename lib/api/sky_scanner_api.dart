...
class AirportApiClient {
  ...
  final Map<String, String> _rapidAPIheaders = {
    "x-rapidapi-key": _api_key,
    "x-rapidapi-host": _baseUrl,
    "content-type": "application/json",
  };
  final Map<String, String> _skyscannerHeaders = {
    "content-type": "application/json",
  ...
  Future<Flights> getDestinationID() async {
    final response = await http.get(
        "http://partners.api.skyscanner.net/apiservices/browseroutes/v1.0/FR/eur/en-US/us/anywhere/anytime/anytime?apikey=prtl6749387986743898559646983194" as Uri,
  ...
  Future<Flights?> fetchFlights(String inboundCity, String outboundCity,
    ...
    try {
      if (inboundDate != null && outboundDate != null) {
        print("outboundCity API : $outboundCity");
        final response = await http.get(
            "http://partners.api.skyscanner.net/apiservices/browsedates/v1.0/TR/TRY/tur/$inboundCity/$outboundCity/$inboundDate/$outboundDate?apikey=prtl6749387986743898559646983194" as Uri,
            headers: _skyscannerHeaders);
        if (response.statusCode == 200) {
        ...
        final response = await http.get(
            "http://partners.api.skyscanner.net/apiservices/browsequotes/v1.0/TR/TRY/tur/$inboundCity/$outboundCity/anytime/anytime?apikey=prtl6749387986743898559646983194" as Uri,
            headers: _skyscannerHeaders);
        if (response.statusCode == 200) {
          return Flights.fromJson(jsonDecode(response.body));
        } else {
  ...
  Future<Airport?> fetchAirport(String city) async {
    try {
      final response = await http.get(
          "https://rapidapi.p.rapidapi.com/apiservices/autosuggest/v1.0/TR/TRY/tur/?query=$city" as Uri,
